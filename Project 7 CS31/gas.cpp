...
void Flatulan::move()
{
    ...
    switch (dir) {
        case 0:
            if (m_row != m_city->player()->row()+1 || m_col != m_city->player()->col()) {
                m_city->determineNewPosition(m_row, m_col, 0);
            }
            break;
        case 1:
            if (m_row != m_city->player()->row() - 1 || m_col != m_city->player()->col()) {
                m_city->determineNewPosition(m_row, m_col, 1);
            }
            break;
        case 2:
            if (m_row != m_city->player()->row() || m_col != m_city->player()->col()+1) {
                m_city->determineNewPosition(m_row, m_col, 2);
            }
            break;
        case 3:
            if (m_row != m_city->player()->row() || m_col != m_city->player()->col()-1) {
                m_city->determineNewPosition(m_row, m_col, 3);
            }
            break;
        default:
            break;
...
{
    int returnVal = 0;
    for (int i = 0; i < m_nFlatulans && m_flatulans[i] != nullptr; i++) {
        if (m_flatulans[i]->col() == c && m_flatulans[i]->row() == r) {
            returnVal++;
        }
    }
...
void City::display() const
{
    ...
    for (int i = 0; i < m_nFlatulans; i++) {
        if (m_flatulans[i] != nullptr) {
            int num = nFlatulansAt(m_flatulans[i]->row(), m_flatulans[i]->col());
            ...
            switch (num) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    grid[m_flatulans[i]->row() - 1][m_flatulans[i]->col() - 1] = '3';
                    break;
                case 4:
                    grid[m_flatulans[i]->row() - 1][m_flatulans[i]->col() - 1] = '4';
                    break;
                case 5:
                    grid[m_flatulans[i]->row() - 1][m_flatulans[i]->col() - 1] = '5';
                    break;
                case 6:
                    grid[m_flatulans[i]->row() - 1][m_flatulans[i]->col() - 1] = '6';
                    break;
                case 7:
                    grid[m_flatulans[i]->row() - 1][m_flatulans[i]->col() - 1] = '7';
                    break;
                case 8:
                    grid[m_flatulans[i]->row() - 1][m_flatulans[i]->col() - 1] = '8';
                    break;
                default:
                    grid[m_flatulans[i]->row() - 1][m_flatulans[i]->col() - 1] = '9';
                    break;
            }
        }
    }
...
void City::preachToFlatulansAroundPlayer()
{
    ...
    // the player.
    for (int i = 0; i < m_nFlatulans; i++) {
        int cDiff = m_flatulans[i]->col() - m_player->col();
        int rDiff = m_flatulans[i]->row() - m_player->row();
        if ((cDiff <=1 && cDiff >= -1) && (rDiff <= 1 && rDiff >= -1)) {
